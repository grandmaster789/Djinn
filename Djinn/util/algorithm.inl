...
namespace djinn::util {
	...
	bool contains_all(const C& container_a, const C& container_b) {
		for (const auto& value : container_b)
			if (!contains(container_a, value)) return false;
	...
	template <typename C, typename E>
	typename C::const_iterator binary_find(const C& container, const E& value) {
		using std::begin;
		using std::end;
	...
	void erase(C& container, const E& value) {
		...
		if (it != std::end(container)) container.erase(it);
	}
	...
	void copy_if(const C& source, C& destination, P&& predicateFn) {
		...
		copy_if(begin(source), end(source), back_inserter(destination), predicateFn);
	}
	...
	void unique(C& container) {
		...
		auto it = unique(begin(container), end(container));
	...
	namespace detail {
		...
		std::optional<typename C::value_type>
		    prefer_impl(const C& options, const T& candidate, const Vs&... vs) {
			static_assert(std::is_same_v<typename C::value_type, T>);

			if (contains(options, candidate)) return candidate;

			if constexpr (sizeof...(Vs) > 0) { return prefer_impl(options, vs...); }
	...
	std::optional<typename tContainer::value_type>
		...
		if constexpr (sizeof...(tValues) == 0) { return std::nullopt; }
	...
	template <typename T, typename It, typename>
	constexpr auto enumerate(T&& iterable) {
		...
		return IteratorWrapper{std::forward<T>(iterable)};
	}
}  // namespace djinn::util
