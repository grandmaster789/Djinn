...
namespace djinn::util {
	...
	WinException::WinException(): mErrorCode(::GetLastError()) {
		mDescription = winErrorToString(mErrorCode);
	}

	WinException::WinException(DWORD errorCode): mErrorCode(errorCode) {
		mDescription = winErrorToString(mErrorCode);
	}
	...
	HresultException::HresultException(HRESULT result): mResult(result) {}
	...
	void throwIfFailed(HRESULT result) {
		if (result != S_OK) throw HresultException(result);
	}
}  // namespace djinn::util
