...
namespace djinn::util::reflect {
    namespace detail {
        ...
        constexpr auto is_brace_constructible_impl(std::index_sequence<Is...>, T*) -> decltype(
            T{g_Wildcard<Is>...},
            std::
                true_type{});  // comma expression, yield std::true_type iff the first part compiles

        template <size_t... Is>
        constexpr std::false_type is_brace_constructible_impl(
            std::index_sequence<Is...>,
            ...);  // use ... to force the lowest matching priority in the overload set
    ...
    template <typename T>
    auto toTuple(T& obj, NumFields<16>) noexcept {
    ...
    template <typename T>
    auto toTuple(T& obj, NumFields<18>) noexcept {
        auto& [f0, f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12, f13, f14, f15, f16, f17]
            = obj;
        return std::tie(
        ...
        if constexpr (std::is_pointer_v<T>) {
            ...
            if (obj != nullptr)
                ...
                throw std::runtime_error(
                    "reflect::forEachField() - Cannot apply callback to nullptr");
        }
            ...
            std::apply([&](auto&&... args) { (callback(args), ...); }, toTuple(obj));
        }
    }
